...
func HouseX(cityId int) {
	...
	// 获取URL
	for _, v := range housePageUrlX(cityId) {
		// 添加队列
		q.AddURL(v)
	}
	// 抓取数据
	c.OnHTML("div.content__article > div.content__list > div.content__list--item", func(e *colly.HTMLElement) {
		...
		// 插入社区信息
		insertCommunity(cityId, districtId, bizcircleId, communityId, communityName)
	})
	//
...
// 房源分页URL
func housePageUrlX(cityId int) []string {
	// 数据库
	S := mongodb.GetS()
	defer S.Close()
	// 切片
	...
	for _, v := range result {
		...
		// 当前页面
		currUrl := fmt.Sprintf("https://%s.zu.ke.com/zufang/%s/", strings.ToLower(config.HousePrefixMap[cityId]), bizcircleId)
		// 添加队列
		q.AddURL(currUrl)
	}
	// 抓取数据
...
// 插入小区信息
func insertCommunity(cityId int, districtId string, bizcircleId string, communityId string, communityName string) {
	// 数据校验
	if districtId == "" || bizcircleId == "" || communityId == "" {
		return
	}
	// 数据库
	S := mongodb.GetS()
	defer S.Close()
	// 插入
	res, err := S.GetC("community").Upsert(bson.M{"community_id": communityId}, bson.M{"$set": bson.M{
		"city_id":        cityId,
		"district_id":    districtId,
		"bizcircle_id":   bizcircleId,
		"community_name": communityName,
		"create_time":    helpers.GetTimestamp(),
		"update_time":    helpers.GetTimestamp(),
	}})
	fmt.Println("[Error]:", err, "[Result]:", res, "[communityId]:", communityId)
}
