...
func Detail(cityId int) {
	...
	c.OnHTML("div.content", func(e *colly.HTMLElement) {
		// 房源标题
		titleDom := e.DOM.Find("p.content__title")
		...
		// 发布日期
		postedDateDom := e.DOM.Find("div.content__subtitle")
		...
		// 租金
		rentDom := e.DOM.Find("div.content__aside > p.content__aside--title > span")
		rent, _ := strconv.ParseInt(rentDom.Text(), 10, 64)
		// 付款方式
		paymentDom := e.DOM.Find("div.content__aside > p.content__aside--title")
		...
		// 标签
		tagDom := e.DOM.Find("div.content__aside > p.content__aside--tags > i")
		tagStr := []string{}
		...
		otherDom := e.DOM.Find("div.content__aside > ul.content__aside__list > p.content__article__table > span")
		...
		houseElevator := ""
		// 基本信息
		infoDom := e.DOM.Find("div.content__article > div.content__article__info > ul > li")
...
// 房源详情URL
func houseDetailUrl(cityId int) []string {
	// 数据库
	S := mongodb.GetS()
	defer S.Close()
	// 切片
	var data []string
	// 失败记录
	for _, v := range helpers.GetFailedLogs(5, cityId) {
		data = append(data, v)
	}
	// 时间范围
	...
	// 查询房间
	result := []house{}
	err := S.GetC("house").Find(bson.M{"city_id": cityId, "offline_date": "", "update_time": bson.M{"$lte": endTime}}).All(&result)
	if err != nil {
		return data
	}
	// 循环
	for _, v := range result {
		...
		houseId := v.HouseId
		// 当前页面
		currUrl := fmt.Sprintf("https://%s.zu.ke.com/zufang/%s.html", strings.ToLower(config.HousePrefixMap[cityId]), houseId)
		// 添加
		data = append(data, currUrl)
		//
		fmt.Println("[Add-URL]:", currUrl)
	}
	//
	return data
}
